...
description: Declarative management (vendoring) of external Agent Skills via a SHA-pinned skills.yaml, supporting project and user scope. Use this skill whenever asked to add, update, remove, or verify external skills, to set up skills.yaml-based management in a repository or dotfiles, to edit or troubleshoot skills.yaml / install-skills.sh / renovate.json / sync-skills.yaml, or when the user says things like "install this skill", "update external skills", or "add this to skills.yaml". Also use it to troubleshoot Renovate PR behavior or drift between the manifest and vendored files.
license: MIT
---
...
entry) and installed by `install-skills.sh`. The manifest lives in whatever git
...
## Locating the manifest and its installer

`install-skills.sh` is the canonical name of the installer, but treat
the filename as a convention, not an identifier: deployments may use
another name (older setups used `install.sh` — the previous canonical
name — or a rename such as `setup_skills.sh`), and an installer-looking
script in the current directory (dotfiles bootstraps commonly have an
`install.sh`) may have nothing to do with this scheme. Before any
operation below, locate the manifest and its installer:

1. Find the skills.yaml the operation targets — it may live in a
   different repository than the current working directory (user-scope
   entries typically live in dotfiles).
2. Identify the installer: the script next to that skills.yaml derived
   from this skill's template. Verify by content — its header reads
   "Install (vendor) external Agent Skills declared in skills.yaml" —
   not by filename.
3. Run it by explicit path (e.g. `~/dotfiles/install-skills.sh`). Never
   run a bare `./install.sh` or `./install-skills.sh` on the assumption
   that it is the installer.

Every mention of install-skills.sh below means the installer identified
this way, whatever its filename. (Template copies older than the
self-locating version resolve skills.yaml from the working directory —
run those from the manifest's directory, and consider re-copying the
current template.)
...
- `user`: `~/.claude/skills/` of the machine running install-skills.sh — shared
  across all projects on that machine.

Scope characteristics that shape operations:

- On CI runners (`GITHUB_ACTIONS=true`) install-skills.sh skips user-scope
  entries entirely: CI can vendor and verify only project scope.
- User-scope updates take effect per machine, only when install-skills.sh is
  rerun there. After merging a Renovate bump for a user-scope entry, run
  the manifest's install-skills.sh on each machine (e.g. as part of dotfiles
  apply).
- `install-skills.sh --check` treats a user-scope entry that is not installed
...
| `templates/install-skills.sh` | repository root (`chmod +x install-skills.sh`) |
...
If the name collides with an existing file in the repository, copy the
template under a different name and replace `install-skills.sh` with
that name everywhere in sync-skills.yaml (the `paths` trigger and both
`run` steps).

Then follow the "add" procedure below for each initial dependency, run
the newly copied install-skills.sh, and commit everything together.
...
"Add", replace it in skills.yaml, and run the manifest's install-skills.sh.
...
```bash
<manifest-dir>/install-skills.sh --check
```
...
- **Never edit installed files directly.** The next `install-skills.sh` run will
...
- **User-scope drift is invisible to CI**: a machine that never reruns
  install-skills.sh keeps stale user-scope skills, and nothing fails. Rerun
  install-skills.sh when pulling manifest changes.
